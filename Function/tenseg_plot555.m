function [fig_out] = tenseg_plot555( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
NodeSize = 10; % Size of node marker
...
FontNodes = 10; % Font of node labels
...
BarSurfColor = [0.5, 0.5, 0.5];  % RGB颜色代码，表示黑色
% StringSurfColor = [0, 0, 1];  % RGB颜色代码，表示蓝色
% StringSurfColor = [0.6, 0.4, 0.8];
 StringSurfColor = [0.5, 0.5, 0.5];
    ...
    if LabelBars == 1
        for i = 1:size(B,2)
            text(bar_start_nodes(1,i) + 0.5*B(1,i) + dist_x, ...
                ...
                num2str(i), 'FontSize',FontBars, 'Color', '[0.5, 0.5, 0.5]')
        end
    end
end
...
%         quiver3(string_start_nodes(1,:),string_start_nodes(2,:),string_start_nodes(3,:),S(1,:),S(2,:),S(3,:),'Color', [0.75, 0.58, 0.89],'Autoscale','off','LineWidth',StringWidth);
for k = 1:size(string_start_nodes, 2)
    plot3([string_start_nodes(1,k), string_start_nodes(1,k) + S(1,k)],...
          [string_start_nodes(2,k), string_start_nodes(2,k) + S(2,k)],...
          [string_start_nodes(3,k), string_start_nodes(3,k) + S(3,k)],...
          'Color', '[0.5, 0.5, 0.5]', 'LineWidth', StringWidth);
    hold on;
end
        hold on
...
%           text(N(1,i) + dist_x, N(2,i) + dist_y, N(3,i) + dist_z,...
%             num2str(i), 'FontSize', FontNodes, 'Color', 'r')
%     end
% end
% 显示节点标签
if LabelNodes == 1
    for i = 1:size(N,2)
        % 调整这些值以优化标签位置
        adjusted_dist_x = dist_x + 0.1; % 根据需要调整
        adjusted_dist_y = dist_y + 0.1; % 根据需要调整
        adjusted_dist_z = dist_z + 0.1; % 根据需要调整

        text(N(1,i) + adjusted_dist_x, N(2,i) + adjusted_dist_y, N(3,i) + adjusted_dist_z,...
            num2str(i), 'FontSize', FontNodes, 'Color', 'r')
    end
end
