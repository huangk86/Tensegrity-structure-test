function [fig_out] = tenseg_plot123456( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
BarSurfColor = [0, 0, 0];  % RGB颜色代码，表示黑色
% StringSurfColor = [0, 0, 1];  % RGB颜色代码，表示蓝色
% StringSurfColor = [0.6, 0.4, 0.8];
 StringSurfColor = [0.75, 0.58, 0.89];
...
if LabelNodes == 1
    for i = 1:size(N,2)
        % 调整这些值以优化标签位置
        adjusted_dist_x = dist_x + 0.05; % 根据需要调整
        adjusted_dist_y = dist_y + 0.05; % 根据需要调整
        adjusted_dist_z = dist_z + 0.05; % 根据需要调整
